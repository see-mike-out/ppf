...
function rgbCodeToHsl(_) {
  c = _.replace('rgb(', '').replace(')', '').trim().split(',');
  return rgbToHsl(c[0], c[1], c[2]);
}

function bright_hex(hex, degree) {
...
function bright_rgb(rgb_code, degree) {
  var hsl = rgbCodeToHsl(rgb_code);
  var deg = 1+degree;
  hsl[2] = hsl[2] * deg;
  if (hsl[2] >= 1) hsl[2] = 1;
  else if (hsl[2] <= 0) hsl[2] = 0;
...
/**
 * from https://gist.github.com/mjackson/5311256#file-color-conversion-algorithms-js
